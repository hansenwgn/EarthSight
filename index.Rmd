...
---
class: inverse, center, middle
# The data calls for a pipeline through the analysis

---

### Persisting risks
Inflexibility in both time and space:
  - if we want to shift the location of area of interest, or, say,
  - progress from 2022 to 2023

We have to redo the whole watershed simulation with new data 

So how do we improve temporal and spatial transferability and build a data pipeline? Even achieve automation? 
...
## Are information gaps "closed"? - dealing with probability

 - What if somehow we neglected a 99%-likely flood location and instead sent all our response team to an 80%-likely flood location?
 
The emergency-handling strategy fails under this circumstance. 
 
 - The trikiness of dealing with probability
  - Should we distribute resources in proportion to probability of flood ocurrence?
  - Or should we design a hierarchy of priority of response considering both likelihood and potential harm?
  - A function? Human-added weights? Machine-learned weights?
...
---
## Risks of Real-time flood monitoring 

 - How real-time is real time? Does a 5-seconds delay in data transmission inflict an unrecoverable loss? 
  
 - How to quantify those tolerance thresholds?

 - Accuracy in estimating response time consumed by data collection, data transmission, calculation, human-based decision-making needs management, which incurs extra cost and is the result almost NOT deterministic.
...
ZHA0 N，LIU Y，Hsu F C，et a1．Time series analysis of VIIRS—DNB nighttime 1ights imagery for change detection in urban areas：A case study of devastation in Puerto Rico f而m hurricanes I瑚a and Maria[J]．Applied Geogr印hy，2020，120(1)：102222
